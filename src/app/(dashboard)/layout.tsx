...
export default function DashboardLayout({
  ...
  useEffect(() => {
    if (role) {
      ...
      const isRouteAllowed = 
        ...
        (pathname === '/configuracoes' && activePerms.configuracoes) ||
        (pathname === '/backups' && (role === 'admin' || role === 'mestre')) ||
  ...
  const navItems = [
    ...
    ...(permissions.eficiencia
      ...
      : []),
    ...(role === 'mestre' || role === 'admin' || permissions.role === 'mestre' || permissions.role === 'admin'
      ? [
          {
            href: '/backups',
            label: 'Backups de Dados',
            icon: (
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
